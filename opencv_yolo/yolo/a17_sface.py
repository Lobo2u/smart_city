...
parser.add_argument('--target', '-t', type=str, default='../../data/face.png',
                    help='Usage: Set path to the input image 1 (target face).')
parser.add_argument('--query', '-q', type=str, default='../../data/face1.jpg',
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 얼굴이 감지되지 않은 경우에도 카메라 화면은 표시
        if faces1.shape[0] < 1 or faces2.shape[0] < 1:
            # 얼굴이 없을 때 카메라 영상만 표시
            cv.putText(img2, "No face detected", (10, 30), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cv.imshow("SFace Demo", img2)
